...
function buildPrompt(logs, categories) {
  ...
  const classificationFlow = getClassificationFlow();
  ...
  if (classificationFlow) {
    systemParts.push("", classificationFlow);
  }
...
function applyCustomTemplate(template, logs, categories) {
  ...
  return {
    system:
      "あなたは業務ログの整理専門AIです。指定されたフォーマットで出力してください。",
    user: filled,
  };
}
